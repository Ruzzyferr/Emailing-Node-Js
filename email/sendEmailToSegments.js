...
exports.sendEmailToTopicMembers = async (event, context, callback) => {
    ...
    try {
        let { appId, iysCheck } = requestBody;
        ...
        const { limitCheckResult, remainingLimit } = await checkEmailSendingLimit(appId, topicMails.length);
        if (!limitCheckResult) {
        ...
        if (isScheduled && scheduledDate) {
            // Scheduled email case: save to database and skip SendGrid and IYS check
            await submitEmailTable(emailIdCreated, requestBody, 0, topicMails.length, scheduledDate);
            return responseStatus(200, 'email was successfully scheduled. Your remaining email usage allowance will be: ' + (remainingLimit.remaining - topicMails.length));
        }

        const nowDate = Math.floor(new Date().getTime() / 1000);
        const emailLogs = [];

        let consentedEmails = topicMails;

        if (iysCheck) {
            consentedEmails = await checkIYSConsentList(appId, topicMails);

            if (consentedEmails.statusCode && consentedEmails.statusCode !== 200) {
                console.log("BURAYA BAK: " + JSON.stringify(consentedEmails));
        ...
        await submitEmailTable(emailIdCreated, requestBody, consentedEmails.length, topicMails.length, null, sendGridMessageId);
...
async function checkIYSConsentList(appId, emails) {
    ...
    try {
        ...
        console.log('IYS consent list check error:', error);
        // Hata mesajını uygun bir formatta döndür
        const { status, message } = error.response?.data ?? "BOŞ DÖNÜYOR";
        return responseStatus(status || 500, message || `IYS consent list check error: ${error.message}`);
    }
}
